...
import { EResponseCode, ETypeNotification } from '@/utils/constants';
...
const AuthorizedInstance = (baseURL) => {
  ...
  const onResponseSuccess = (response) => {
    const isExpireToken = response?.data?.code === 9901;
    if (isExpireToken) {
      AuthHelpers.clearTokens();
      navigate(LayoutPaths.Auth);
      showNotification(ETypeNotification.ERROR, 'The token is expired');
      return response;
    } else {
      return response;
    }
